...
def start_mission(
    ...
    # ------------- Call remote worker to start SITL -------------
    payload = {
        "drone_id": drone_id,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Worker start failed: {e}")
    ...
    # record that worker started SITL (use worker response)
    active_missions[drone_id] = {"worker_pid": resp.get("worker_pid"), "sitl_port": sitl_udp}
    print(f"✅ Worker acknowledged SITL start for drone {drone_id}")

    return {
        "message": f"Mission start requested for Drone {drone_id} (worker started SITL)",
        "sitl_port": sitl_udp,
...
def pause_mission(drone_id: int):
    ...
    try:
        r = requests.post(f"{WORKER_URL}/missions/pause", json={"drone_id": drone_id}, timeout=5)
        r.raise_for_status()
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Worker pause failed: {e}")
    return {"message": f"Pause requested for Drone {drone_id}"}
...
def resume_mission(drone_id: int):
    ...
    try:
        r = requests.post(f"{WORKER_URL}/missions/resume", json={"drone_id": drone_id}, timeout=5)
        r.raise_for_status()
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Worker resume failed: {e}")
    return {"message": f"Resume requested for Drone {drone_id}"}
...
def abort_mission(
    ...
    try:
        r = requests.post(f"{WORKER_URL}/missions/abort", json={"drone_id": drone_id}, timeout=5)
        r.raise_for_status()
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Worker abort failed: {e}")
...
# -----------------------------
# Endpoint for worker logs
# -----------------------------
@app.post("/telemetry/logs")
async def receive_logs(request: Request):
    """
    Worker will POST its SITL/stdout logs here for storage/inspection.
    Example payload: { "drone_id": 1, "log": "text line", "level": "info" }
    """
    payload = await request.json()
    drone_id = payload.get("drone_id")
    log = payload.get("log")
    level = payload.get("level", "info")
    # Basic store: print + optionally persist to DB (left minimal)
    print(f"[WORKER LOG][DRONE {drone_id}][{level}] {log}")
    # Optionally create a Telemetry log table and store this — omitted for brevity
    return {"status": "ok"}
